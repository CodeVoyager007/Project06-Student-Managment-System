...
class Student {
    ...
    coursesEnrolled;
    feesAmount;
    active; // Track if student is active
...
let students = []; // Array to store active students
let removedStudents = []; // Array to store removed students
...
async function removeStudent() {
    ...
    let studentIndex = students.findIndex((student) => student.name === studentToRemove);
...
async function showRemovedStudents() {
    if (removedStudents.length !== 0) {
        ...
        removedStudents.forEach((student) => {
            console.log(chalk.yellow(`\t- ${student.name}: ${student.removalReason}`));
        });
        console.log("\n");
    }
    else {
